	...
	UseModule MyFont
		UseModule Mysplitter
			...
			*textRow=*header4\AddRow("width"):styleEditCellNumber(*textRow\GetCell(1))
			...
			Global *splitter.MySplitter=MySplitterCreateSplitter(0,0,WindowWidth(window),WindowHeight(window),container,panel,#MYSPLITTER_FLAGS_FIRSTFIXED|#MYSPLITTER_FLAGS_VERTICAL)
			*splitter\SetValue(300)
			
			*styleTree\setRedraw(#True)
			*preview\setRedraw(#True)
			
			Procedure Resize()
				*splitter\Resize(#PB_Ignore,
				             #PB_Ignore,
				             WindowWidth(window),
				             WindowHeight(window))
				ResizeGadget(editor,
