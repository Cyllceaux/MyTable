	...
	UseModule MyFont
		...
		Procedure getStyle(*obj.MyTableObject)
			...
			Select GetGadgetState(pmGruppe)
				...
				Case 4
					If *obj\getType()=My::#MY_TYPE_CELL
						*cell=*obj
						ProcedureReturn getStyle(*cell\getTable())
					ElseIf *obj\getType()=My::#MY_TYPE_ROW
						*row=*obj
						ProcedureReturn *row\getElementSelectedStyle()
					ElseIf *obj\getType()=My::#MY_TYPE_COL
						*col=*obj
						ProcedureReturn *col\getElementSelectedStyle()
					ElseIf *obj\getType()=My::#MY_TYPE_TABLE Or *obj\getType()=My::#MY_TYPE_TREE Or *obj\getType()=My::#MY_TYPE_GRID	
						*table=*obj			
		...
		Procedure changeFlags(fl)
			...
			Protected *nfont.MyFont=0
			Protected *font.MyFont=0
			Protected *style.MyTableStyle=0
			*selectedTable\GetSelectedCells(cells())
			*selectedTable\GetSelectedRows(rows())
			*selectedTable\GetSelectedCols(cols())
			...
			ForEach cells()
				...
				*font=*style\GetFont()
				...
				Else
					flags!fl
				EndIf
				*nfont=MyFontCreateFont(*font\GetName(),*font\GetSize(),flags)
				*style\SetFont(*nfont)		
			Next
			ForEach rows()
				found=#True
		...
		Procedure changeFont(font.s,size.i)
			...
			*selectedTable\SetRedraw(#False)
			ForEach cells()
				...
				*font=*style\GetFont()
				flags=*font\GetFlags()
				*nfont=MyFontCreateFont(font,size,flags)
				*style\SetFont(*nfont)		
			Next
			ForEach rows()
				found=#True
		...
		Procedure ButtonUpdate(*style.MyTableStyle)
			Protected *font.MyFont=*style\GetFont()
